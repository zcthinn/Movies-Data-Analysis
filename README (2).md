...
## 🗂️ Data
The full dataset is ~1M rows / 151MB — too large for GitHub's file limits. This repo includes:
- `data/movies_dataset_sample.csv` — a 30,000-row stratified sample (preserves genre proportions), small enough to version and run the notebook end-to-end out of the box.
- The full dataset can be regenerated or re-downloaded from **[add your source link here — e.g. Kaggle/Google Drive]**, then re-sampled locally with `make_sample.py` if needed.

All figures and insights quoted in this README were computed on the **full 1M-row dataset**; results on the sample file will be directionally the same but numerically slightly different due to sample size.
...
├── data/
│   └── movies_dataset_sample.csv      # 30K-row sample (full dataset too large for GitHub)
├── images/                            # Exported chart screenshots for README
├── make_sample.py                     # Regenerates the sample from the full dataset
